    ...
    try {
        ...
        if (json.status === 'success') {
            ...
            json.data.forEach(c => {
                ...
                const badge = `<span class="badge ${sCls}">${c.status}</span>`;
...
function initDeptEvents() {
    ...
    document.getElementById('saveDeptBtn').addEventListener('click', async () => {
        ...
        try {
            const payload = { action: id ? 'edit' : 'add', is_admin: user.is_admin, name, description: desc };
            if (id) payload.id = id;
...
function loadDepartmentsPanel() {
    ...
    departmentsData.forEach((d, idx) => {
        ...
        tr.innerHTML = `
            ...
            <td><span class="badge Resolved" style="padding:.2rem .5rem">${d.total_resolved} Checkins</span></td>
            <td><span class="sla-badge ${d.total_sla_breached > 0 ? 'breach' : ''}">${d.total_sla_breached} Breaches</span></td>
            <td class="actions-cell">
                <button class="act-btn green" onclick="editDept(${d.id}, '${d.name.replace(/'/g, "\\'")}', '${(d.description || '').replace(/'/g, "\\'")}')">Edit</button>
